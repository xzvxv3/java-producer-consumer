...
public class LockMain {
    ...
    public static void main(String[] args) {
        BoundedQueue queue = new LockBoundedQueue(2);
    ...
    private static void startProducer(BoundedQueue queue, List<Thread> threads) {
        ...
        for(int i = 1; i <= 3; i++) {
            Thread producer = new Thread(new LockProducer(queue, new Order(i, menus[new Random().nextInt(menus.length)])), "생산자" + i);
    ...
    private static void startConsumer(BoundedQueue queue, List<Thread> threads) {
        log("============ [소비자 시작] ============");
        for(int i = 1; i <= 3; i++) {
            Thread consumer = new Thread(new LockConsumer(queue), "소비자" + i);
